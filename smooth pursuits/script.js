...
function updatePreview() {
  ...
  display.speedValue.textContent = formatSpeed(config.speed);
  buttons.finishEarly.disabled = completedSets.length === 0;
  display.setupPreview.textContent =
    `${formatDifficulty(config.difficulty)}, ${formatPattern(getPatternForDifficulty(config.difficulty))}, ${formatTargetType(config.targetType)}, ${formatBackground(config.backgroundMode)}, ${config.duration} seconds per set, speed ${formatSpeed(config.speed)}, ${config.sets} set${config.sets > 1 ? "s" : ""}, ${config.position.toLowerCase()} on ${config.surface.toLowerCase()} surface.`;
}
...
function calculatePoint(progress, config) {
  ...
  const speedMultiplier = Math.max(config.speed / 20, 0.2);
...
function buildDocumentationParagraph() {
  ...
  const setDetails = completedSets.map((set) =>
    `Set ${set.setNumber}: ${set.duration}s, speed ${formatSpeed(set.speed)}, ${formatPattern(getPatternForDifficulty(set.difficulty)).toLowerCase()}, ${formatTargetType(set.targetType).toLowerCase()}, ${set.position.toLowerCase()} on ${set.surface.toLowerCase()} surface, dizziness ${set.dizziness}/10`
  ).join("; ");
  const summary =
